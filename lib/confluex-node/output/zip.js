...
function zipPathForOutputRoot (outputRoot) {
  return `${outputRoot}.zip`
}

async function assertZipPathAvailable (zipPath) {
  try {
...
async function createZipFromRoot (outputRoot, zipPath = zipPathForOutputRoot(outputRoot)) {
...
async function collectFileEntries (root, relativeDirectory) {
...
function validateZipRelativePath (relativePath) {
  if (
...
function centralDirectoryHeader (nameBytes, size, crc, localHeaderOffset) {
