  ...
  try {
    ...
    const result = runCommand('bats', ['--tap', liveRegressionPath], {
      ...
      env: {
        ...
        CONFLUEX_SELFTEST_CONFLUENCE_USERNAME: target.username,
        CONFLUEX_SELFTEST_CONFLUENCE_PASSWORD: target.password,
        CONFLUEX_CONFLUENCE_BASE_URL: target.baseUrl,
        CONFLUEX_CONFLUENCE_USERNAME: target.username,
        CONFLUEX_CONFLUENCE_PASSWORD: target.password
      }
    })
    const tap = [
