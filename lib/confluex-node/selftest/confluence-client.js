...
function createSelftestConfluenceClient (options = {}) {
  ...
  const token = requiredString(options, 'token')
  const authorization = `Bearer ${token}`

  return {
    async resetStand () {
      try {
        const response = await requestRaw(baseUrl, authorization, 'POST', '/__confluence-stand/reset', {})
        const body = parseSuccessfulJsonObject(response)
        return body.status === 'reset' && body.ready === true ? { state: 'passed' } : { state: 'failed' }
      } catch {
        return { state: 'failed' }
      }
    },
