...
const emptyRejectedOptions = new Set(['--out', '--log-file', '--install-dir', '--encryption-key'])
const positiveIntegerOptions = new Set(['--max-pages', '--max-download-mib', '--max-find-candidates'])

function validateCommandInvocation (commandName, argv) {
...
function isValidEffectiveValue (optionToken, value) {
  ...
  if (optionToken === '--login') {
    return value !== '' && !/[:\t\n\r]/.test(value)
  }

  if (optionToken === '--password') {
    return value !== '' && !/[\t\n\r]/.test(value)
  }
