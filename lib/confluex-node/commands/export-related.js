...
const { assertZipPathAvailable, createZipFromRoot, zipPathForOutputRoot } = require('../output/zip')
...
async function runRootMetadataFailure (command, pageId, outputRoot, options, preparedLog, dependencies, initialDownloadedBytes) {
  try {
    ...
    return finalizeRunResult(command, pageId, outputRoot, options, finalStatus, preparedLog)
  } catch {
    return {
      exitCode: 4,
      stdout: '',
...
async function runBasicPlan (metadata, outputRoot, options, preparedLog, dependencies, initialDownloadedBytes) {
  try {
    ...
    await writeRunReportSet(outputRoot, reportTexts)
    return finalizeRunResult('plan', pageId, outputRoot, options, finalStatus, preparedLog)
  } catch {
    return {
      exitCode: 4,
      stdout: '',
...
async function finalizeRunResult (command, pageId, outputRoot, options, finalStatus, preparedLog) {
  return successfulPlainRootRunResult(command, pageId, outputRoot, options, finalStatus, preparedLog)
}

async function successfulPlainRootRunResult (command, pageId, outputRoot, options, finalStatus, preparedLog) {
...
async function packageZipIfRequested (command, outputRoot, options) {
...
function isBasicExportOptions (command, options) {
  ...
  return command === 'export' &&
    hasOnlyFlags(options, ['--safe', '--keep-metadata', '--no-fail-fast', '--resume', '--zip']) &&
...
function isBasicPlanOptions (command, options) {
  const allowedValues = ['--page-id', '--out', '--log-file', '--sleep-ms', MAX_FIND_CANDIDATES_OPTION, LINK_DEPTH_OPTION]
  return command === 'plan' &&
    hasOnlyFlags(options, ['--keep-metadata', '--safe', '--no-fail-fast']) &&
    Object.keys(options.values).every(token => allowedValues.includes(token) || BOUNDED_VALUE_OPTIONS.includes(token))
}
...
function isZipRequested (command, options) {
...
function hasSafeProfile (options) {
  return options.flags.includes('--safe')
}
...
async function payloadFolderEntriesAreReusable (folderPath, pagePayloadFormat) {
  ...
  for (const entry of entries) {
    if (!allowedNames.has(entry.name) || entry.name === 'page.html') {
      return false
    }
