...
async function runDoctorCommand (options, commandDependencies = {}) {
  ...
  try {
    ...
    const pageAccess = await determinePageAccess(options, pageAccessChecker)
...
async function determinePageAccess (options, pageAccessChecker) {
  if (!Object.prototype.hasOwnProperty.call(options.values, '--page-id')) {
    return { state: 'skipped' }
  }

  const result = await pageAccessChecker(options.values['--page-id'])
  if (result.state === 'ok') {
    return {
      state: 'ok',
...
function pageIdentityLines (pageAccess) {
  ...
  return [`page_identity=${pageAccess.identity}`]
}
...
function determineNextActions (dependencyStates, pageAccess, encryptionRecipient) {
  ...
  if (stateByLabel.get('bats_cli') === 'absent') {
    actions.add('install_bats_cli')
  }
