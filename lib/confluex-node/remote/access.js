...
function resolveRemoteAccessContext (env = process.env) {
  ...
  const username = env.CONFLUEX_CONFLUENCE_USERNAME
  const password = env.CONFLUEX_CONFLUENCE_PASSWORD

  if (!validEnvironmentValue(baseUrlValue) || !validEnvironmentValue(username) || !validEnvironmentValue(password)) {
    return { usable: false }
  }
...
async function checkRootPageAccess (pageId, env = process.env) {
