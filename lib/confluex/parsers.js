...
function inspectChildren (filePath) {
  const data = JSON.parse(readText(filePath))
  const ids = collectChildrenIds(data)
  let partialListingDetail = ''

  function notePartialListing (detail) {
    if (!partialListingDetail) {
      partialListingDetail = detail
    }
  }
  ...
  function walk (value) {
    ...
    if (value.hasMore === true) {
      notePartialListing('hasMore_true')
    }

    if (typeof value.next === 'string' && value.next.trim()) {
      notePartialListing('next_link_present')
    }

    if (typeof value.cursor === 'string' && value.cursor.trim()) {
      notePartialListing('cursor_present')
    }
  ...
  if (partialListingDetail) {
    process.stdout.write(`flag${US}partial_listing${US}${partialListingDetail}\n`)
  }
}
...
function extractLinks (filePath, currentSpace) {
  ...
  while ((match = contentEntityRe.exec(scanXml))) {
    emitUnique(rows, ['content_id', match[1]])
  }
  ...
  while ((match = pageRe.exec(scanXml))) {
    ...
    if (contentId) {
      emitUnique(rows, ['content_id', contentId])
      continue
    }
    ...
    emitUnique(rows, ['page_ref', space, title])
  }
  ...
  while ((match = pageParamRe.exec(scanXml))) {
    ...
    if (title) {
      emitUnique(rows, ['macro_param', space, title])
    }
  }
  ...
  while ((match = riUrlRe.exec(scanXml))) {
    ...
    if (pageId) {
      emitUnique(rows, ['ri_url_page_id', pageId])
      continue
    }

    const titleRef = internalTitleRefFromUrl(rawUrl, currentSpace)
    if (titleRef && titleRef.title) {
      emitUnique(rows, ['ri_url_space_title', titleRef.space, titleRef.title])
      continue
    }
  ...
  while ((match = hrefRe.exec(scanXml))) {
    ...
    if (pageId) {
      emitUnique(rows, ['href_page_id', pageId])
      continue
    }

    const titleRef = internalTitleRefFromUrl(href, currentSpace)
    if (titleRef && titleRef.title) {
      emitUnique(rows, ['href_space_title', titleRef.space, titleRef.title])
      continue
    }
