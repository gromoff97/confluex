...
export type SelectedEnvFile = {
  path: string | null
  values: Map<string, string>
}

export function parseEnvFile (bytes: Buffer | string): Map<string, string> {
...
function invalidKeyCharacter (key: string): boolean {
  return key.includes('=') ||
    key.includes('\u0000') ||
    key.includes('\n') ||
    key.includes('\r')
...
export function loadSelectedEnvFile (cwd: string, explicitPath?: string): SelectedEnvFile {
...
function defaultEnvFilePath (cwd: string): string | null {
