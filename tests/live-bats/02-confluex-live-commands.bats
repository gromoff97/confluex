...
@test "export --page-format html emits HTML payloads and attachments according to the docs-defined canonical graph expectations" {
  rm -rf "$CONFLUEX_LIVE_REPORT_ROOT/export-html"
  run env HOME="$CONFLUEX_LIVE_CLI_HOME" \
    "$LIVE_CONFLUEX_REPO_ROOT/confluex" \
    export \
    --page-id "$ROOT_PAGE_ID" \
    --page-format html \
    --out "$CONFLUEX_LIVE_REPORT_ROOT/export-html"
  [ "$status" -eq 0 ]
...
const reportRoot = process.argv[1];
const identities = JSON.parse(fs.readFileSync(process.argv[2], "utf8"));
const expected = JSON.parse(fs.readFileSync(process.argv[3], "utf8")).export_html;
const expectedDir = process.argv[4];
...
for (const [logicalName, filename] of Object.entries(expected.payload_files)) {
  ...
  const folder = row[3];
  const expectedPayloadPath = path.join(reportRoot, folder, filename);
  if (!fs.existsSync(expectedPayloadPath)) problems.push(`payload:missing:${logicalName}:${filename}`);
  if (fs.existsSync(expectedPayloadPath)) {
    const actualPayload = fs.readFileSync(expectedPayloadPath);
    const expectedPayload = fs.readFileSync(path.join(expectedDir, "payloads", "html", `${logicalName}.page.html`));
    if (!actualPayload.equals(expectedPayload)) problems.push(`payload:bytes:${logicalName}:${filename}`);
  }
}

for (const [logicalName, filename] of Object.entries(expected.forbidden_payload_files)) {
  const pageId = String(identities[logicalName].page_id);
  const row = manifest.rows.find((candidate) => candidate[0] === pageId);
  if (!row) {
    problems.push(`manifest:missing:${logicalName}`);
  ...
  const folder = row[3];
  const forbiddenPath = path.join(reportRoot, folder, filename);
  if (fs.existsSync(forbiddenPath)) problems.push(`payload:unexpected:${logicalName}:${filename}`);
}
...
' "$CONFLUEX_LIVE_REPORT_ROOT/export-html" "$CONFLUEX_LIVE_IDENTITY_FILE" "$EXPECTATIONS_FILE" "$CONFLUEX_LIVE_EXPECTED_DIR"
  [ "$status" -eq 0 ] || live_fail_test "$output"
}
