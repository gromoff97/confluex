...
test('critical export with root metadata fails closed after retained report set', async () => {
  const out = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-critical-export-')), 'out')
  ...
  assert.equal(result.stderr, '')
  assert.match(result.stdout, /^RUN_COMPLETE final_status=policy_failed artifact=/m)
  const summary = fs.readFileSync(path.join(out, 'summary.txt'), 'utf8')
  assert.match(summary, /^final_status=policy_failed$/m)
  assert.match(summary, /^blocking_reasons=scope_findings,failed_operations$/m)
})
...
test('basic encrypted plan finalizes report and selects encrypted archive', async () => {
  const out = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-basic-encrypted-plan-')), 'out')
  ...
  const result = await runExportRelatedCommand('plan', options({
    ...
    encryptOutputRoot: async (outputRoot, recipient) => {
      assert.equal(outputRoot, out)
      assert.equal(recipient, 'recipient')
      encryptedSummary = fs.readFileSync(path.join(outputRoot, 'summary.txt'), 'utf8')
      fs.writeFileSync(archivePath, 'encrypted archive')
      fs.rmSync(outputRoot, { recursive: true, force: true })
      return { state: 'ok', archivePath }
    }
  })

  assert.equal(result.exitCode, 0)
  assert.equal(result.stderr, '')
  assert.match(result.stdout, /^RUN_PHASE phase=encryption$/m)
  assert.match(result.stdout, new RegExp(`^RUN_COMPLETE final_status=success artifact=${escapeRegExp(quotePathString(archivePath))}$`, 'm'))
  assert.equal(fs.existsSync(out), false)
  assert.equal(fs.readFileSync(archivePath, 'utf8'), 'encrypted archive')
  assert.match(encryptedSummary, /^final_status=success$/m)
  assert.match(encryptedSummary, /^encryption_enabled=1$/m)
  assert.match(encryptedSummary, /^encryption_successful=1$/m)
})

test('basic encrypted export finalizes report and selects encrypted archive', async () => {
  const out = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-basic-encrypted-export-')), 'out')
  const archivePath = `${out}.tar.gz.gpg`
  const payload = '# Root Page\n\nNo links here.\n'
  let encryptedSummary = ''
  let observedPayload = ''
  ...
  const result = await runExportRelatedCommand('export', options({
    ...
    encryptOutputRoot: async (outputRoot, recipient) => {
      ...
      assert.equal(recipient, 'recipient')
      encryptedSummary = fs.readFileSync(path.join(outputRoot, 'summary.txt'), 'utf8')
      observedPayload = fs.readFileSync(path.join(outputRoot, 'pages', 'space__4358', 'page__123', 'page.md'), 'utf8')
      fs.writeFileSync(archivePath, 'encrypted archive')
      fs.rmSync(outputRoot, { recursive: true, force: true })
      return { state: 'ok', archivePath }
    }
  ...
  assert.match(result.stdout, new RegExp(`^RUN_COMPLETE final_status=success artifact=${escapeRegExp(quotePathString(archivePath))}$`, 'm'))
  assert.equal(fs.existsSync(out), false)
  assert.equal(observedPayload, normalizeMarkdownPayload(payload))
  assert.match(encryptedSummary, /^command=export$/m)
  assert.match(encryptedSummary, /^page_payload_format=md$/m)
  assert.match(encryptedSummary, /^final_status=success$/m)
  assert.match(encryptedSummary, /^encryption_enabled=1$/m)
  assert.match(encryptedSummary, /^encryption_successful=1$/m)
})

test('basic encrypted plan max-download does not begin encryption phase', async () => {
  const out = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-encrypted-plan-incomplete-')), 'out')
  ...
  const result = await runExportRelatedCommand('plan', options({
    ...
    encryptOutputRoot: async () => {
      throw new Error('encryption must not start for incomplete run')
    }
  })

  assert.equal(result.exitCode, 3)
  assert.equal(result.stderr, '')
  assert.doesNotMatch(result.stdout, /^RUN_PHASE phase=encryption$/m)
  assert.equal(fs.existsSync(out), true)
  const summary = fs.readFileSync(path.join(out, 'summary.txt'), 'utf8')
  assert.match(summary, /^final_status=incomplete$/m)
  assert.match(summary, /^interrupt_reason=max_download_limit_reached$/m)
  assert.match(summary, /^encryption_enabled=1$/m)
  assert.match(summary, /^encryption_successful=0$/m)
})

test('basic encrypted plan preserves plaintext root on standard encryption failure', async () => {
  const out = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-encrypted-plan-failure-')), 'out')
  ...
  const result = await runExportRelatedCommand('plan', options({
    ...
    encryptOutputRoot: async () => ({ state: 'failed' })
  })

  assert.equal(result.exitCode, 5)
  assert.equal(result.stderr, '')
  assert.match(result.stdout, /^RUN_PHASE phase=encryption$/m)
  assert.match(result.stdout, new RegExp(`^RUN_COMPLETE final_status=encryption_failed artifact=${escapeRegExp(quotePathString(out))}$`, 'm'))
  assert.equal(fs.existsSync(out), true)
  const summary = fs.readFileSync(path.join(out, 'summary.txt'), 'utf8')
  assert.match(summary, /^final_status=encryption_failed$/m)
  assert.match(summary, /^encryption_enabled=1$/m)
  assert.match(summary, /^encryption_successful=0$/m)
})

test('basic confidential plan finalizes encrypted archive with normalized fingerprint', async () => {
  const out = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-confidential-plan-')), 'out')
  const archivePath = `${out}.tar.gz.gpg`
  ...
  const result = await runExportRelatedCommand('plan', options({
    ...
    recipientValidator: recipient => {
      assert.equal(recipient, fingerprint.toUpperCase())
      return true
    },
    checkRootPageAccess: async () => ({
      state: 'ok',
...
test('basic confidential plan applies critical policy before successful encryption', async () => {
  const out = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-confidential-policy-plan-')), 'out')
  const archivePath = `${out}.tar.gz.gpg`
  const fingerprint = '0123456789abcdef0123456789abcdef01234567'
  let encryptedSummary = ''
  ...
  const result = await runExportRelatedCommand('plan', options({
    ...
    encryptOutputRoot: async outputRoot => {
      encryptedSummary = fs.readFileSync(path.join(outputRoot, 'summary.txt'), 'utf8')
      fs.writeFileSync(archivePath, 'encrypted archive')
      fs.rmSync(outputRoot, { recursive: true, force: true })
      return { state: 'ok', archivePath }
  ...
  assert.equal(result.exitCode, 2)
  assert.equal(result.stderr, '')
  assert.match(result.stdout, new RegExp(`^RUN_COMPLETE final_status=policy_failed artifact=${escapeRegExp(quotePathString(archivePath))}$`, 'm'))
  assert.match(encryptedSummary, /^final_status=policy_failed$/m)
  assert.match(encryptedSummary, /^blocking_reasons=scope_findings$/m)
})

test('basic confidential plan removes plaintext root and retains status sidecar on encryption failure', async () => {
  const out = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-confidential-failure-plan-')), 'out')
  const statusSidecar = `${out}.status.txt`
  const fingerprint = '0123456789abcdef0123456789abcdef01234567'
  ...
  assert.equal(result.stderr, '')
  assert.match(result.stdout, /^RUN_PHASE phase=encryption$/m)
  assert.match(result.stdout, new RegExp(`^RUN_COMPLETE final_status=encryption_failed artifact=${escapeRegExp(quotePathString(statusSidecar))}$`, 'm'))
  assert.equal(fs.existsSync(out), false)
  assert.equal(fs.readFileSync(statusSidecar, 'utf8'), 'final_status=encryption_failed\n')
  assert.equal(fs.existsSync(`${out}.tar.gz.gpg`), false)
  assert.equal(fs.existsSync(`${out}.tar.gz.gpg.txt`), false)
})

test('basic confidential plan max-download does not begin encryption phase', async () => {
  const out = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-confidential-incomplete-plan-')), 'out')
  const fingerprint = '0123456789abcdef0123456789abcdef01234567'
  ...
  assert.doesNotMatch(result.stdout, /^RUN_PHASE phase=encryption$/m)
  assert.equal(fs.existsSync(out), true)
  assert.equal(fs.existsSync(`${out}.status.txt`), false)
  const summary = fs.readFileSync(path.join(out, 'summary.txt'), 'utf8')
  assert.match(summary, /^final_status=incomplete$/m)
  assert.match(summary, /^interrupt_reason=max_download_limit_reached$/m)
  assert.match(summary, /^encryption_enabled=1$/m)
...
test('basic confidential plan warns when persistent log is outside plaintext cleanup', async () => {
  const parent = fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-confidential-log-plan-'))
  const out = path.join(parent, 'out')
  const logFile = path.join(parent, 'run.log')
  const archivePath = `${out}.tar.gz.gpg`
  const fingerprint = '0123456789abcdef0123456789abcdef01234567'
  ...
  assert.equal(result.exitCode, 0)
  assert.equal(result.stderr, 'WARNING: confidential_log_file_outside_plaintext_cleanup\n')
...
test('critical plan with root metadata fails closed after retained report set', async () => {
  const out = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'confluex-critical-plan-')), 'out')
...
test('encrypted export rejects missing recipient before root page preflight', async () => {
  const result = await runExportRelatedCommand('export', options({
    flags: ['--encrypt']
  }), {
    store: { readDefaultEncryptionKey: () => null },
    checkRootPageAccess: async () => {
      throw new Error('root page preflight must not run')
    }
  })
...
test('encrypted plan rejects failed gpg recipient validation before root page preflight', async () => {
  const result = await runExportRelatedCommand('plan', options({
    flags: ['--encrypt'],
    values: {
      '--page-id': '123',
  ...
  assert.equal(result.exitCode, 1)
  assert.equal(result.stdout, '')
  assert.equal(result.stderr, 'ERROR: validation_failed FR-0108\n')
})

test('confidential export rejects non-fingerprint recipient before gpg validation', async () => {
  const result = await runExportRelatedCommand('export', options({
    flags: ['--confidential'],
    values: {
      '--page-id': '123',
    ...
    store: { readDefaultEncryptionKey: () => null },
    recipientValidator: () => {
      throw new Error('recipient validator must not run')
    },
    checkRootPageAccess: async () => {
      throw new Error('root page preflight must not run')
    }
  ...
  assert.equal(result.exitCode, 1)
  assert.equal(result.stdout, '')
  assert.equal(result.stderr, 'ERROR: validation_failed FR-0025\n')
})

test('valid encryption recipient proceeds to root page preflight', async () => {
  const order = []
  ...
  const result = await runExportRelatedCommand('export', options({
    ...
    store: { readDefaultEncryptionKey: () => null },
    recipientValidator: recipient => {
      order.push(`recipient:${recipient}`)
      return true
    },
    checkRootPageAccess: async pageId => {
  ...
  assert.deepEqual(order, ['recipient:recipient', 'page:123'])
  assert.equal(result.exitCode, 1)
  assert.equal(result.stdout, '')
  assert.equal(result.stderr, 'ERROR: validation_failed FR-0017 --page-id 123\n')
})
